...
private fun defaultClanColorItems(): Map<String, GuiItemConfig> = mapOf(
    ...
    "statusInfo" to GuiItemConfig(slot = 18, material = "REPEATER", name = "&#FFD700✦ Статус метки", lore = listOf("", "&#9EFC65 «Когда показывать»", " &7- &fВключена — метка отображается.", " &7- &fВыключена — метка полностью скрыта.", "", "&#FF8702➥ &fВыберите состояние ниже")),
    "enabled" to GuiItemConfig(slot = 21, material = "BEACON", name = "&#5EFD7D✦ Включена", lore = listOf("", "&#9EFC65 «Состояние»", " &7- &fСтатус: {status}", "", "&#FC65DF «Описание»", " &7- &fМетка активна для соклановцев.", "", "&#FF8702➥ &fНажмите, &eЛКМ &fчтобы включить")),
    "disabled" to GuiItemConfig(slot = 23, material = "BARRIER", name = "&#FC3737✦ Выключена", lore = listOf("", "&#9EFC65 «Состояние»", " &7- &fСтатус: {status}", "", "&#FC65DF «Описание»", " &7- &fМетка полностью отключена.", " &7- &fВыбранные настройки сохраняются.", "", "&#FF8702➥ &fНажмите, &eЛКМ &fчтобы выключить")),
    ...
    "reset" to GuiItemConfig(slot = 40, material = "LAVA_BUCKET", name = "&#FC65DF✦ Сбросить метку", lore = listOf("", "&#9EFC65 «Действие»", " &7- &fВернуть стандартный статус,", " &7- &fтип и цвет метки.", "", "&#FF8702➥ &fНажмите, &eЛКМ &fчтобы сбросить")),
    "back" to GuiItemConfig(slot = 49, material = "RED_CANDLE", name = "&#FC3737⏎ Вернуться назад", lore = listOf("", "&#FC65DF «Переход»", " &7- &fОткрывает настройки клана.", "", "&#FF8702➥ &fНажмите, &eЛКМ &fчтобы вернуться"))
)
...
private fun colorItem(slot: Int, material: String): GuiItemConfig = GuiItemConfig(
    ...
    lore = listOf("", "&#9EFC65 «Цвет метки»", " &7- &fТип: &e{type}", " &7- &fСтатус: {status}", "", "&#FC65DF «Описание»", " &7- &fЭтот цвет увидят соклановцы.", "", "&#FF8702➥ &fНажмите, &eЛКМ &fчтобы применить")
)
...
private fun historyEntry(material: String, name: String): GuiItemConfig = GuiItemConfig(
    material = material,
    name = name,
    lore = listOf("", "&#9EFC65 «Детали операции»", " &7- &fТип: {operation}", " &7- &fИнициатор: &e{player}", "", "&#9EFC65 «Сумма»", " &7- &fОперация: {signed_amount} ⛁", "", "&#5EA9FD «Время операции»", " &7- &fДата: &b{date}", " &7- &fВремя: &b{time}")
)
