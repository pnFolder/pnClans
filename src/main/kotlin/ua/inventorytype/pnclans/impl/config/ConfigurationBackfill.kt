...
internal object ConfigurationBackfill {
    ...
    /**
     * `autoUpdate: false` used to be generated by default, so it cannot be treated as an intentional
     * opt-out during the channel migration. On the first migration only (identified by missing
     * updateChannel), old update flags are removed. After updateChannel exists, an administrator may
     * explicitly add `autoUpdate: false` and it will remain untouched on every future reload.
     */
