    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        try {
            ...
            ErrorReporter.report(
                plugin = plugin,
                context = "Command Execution: /clan ${args.joinToString(" ")}",
                throwable = throwable,
                player = sender,
                extraData = mapOf("Arguments" to args.joinToString(" "))
            )
            sender.sendMessage("§cПроизошла ошибка при выполнении команды. Отчет отправлен администраторам.")
        }
